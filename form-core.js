...
(function (global) {
  ...
  function buildReportHTML(a) {
    ...
    return '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="utf-8"><title>职业测评自报告</title><style>'
      + 'body{font-family:-apple-system,"PingFang SC","Microsoft YaHei",sans-serif;max-width:780px;margin:28px auto;padding:0 18px;color:#1f2933;line-height:1.7}'
      + 'h1{font-size:23px;border-bottom:3px solid #6c5ce7;padding-bottom:8px;margin-bottom:4px} h2{font-size:17px;margin-top:30px;color:#4834d4} h3{font-size:15px;margin:20px 0 8px;color:#6c5ce7}'
      + 'table{width:100%;border-collapse:collapse;margin:8px 0;font-size:14px} td{border:1px solid #e4e7eb;padding:7px 9px;text-align:left}'
      + '.rbar{background:#eef0f3;border-radius:6px;height:11px;width:170px;overflow:hidden} .rbar span{display:block;height:100%;background:linear-gradient(90deg,#6c5ce7,#a29bfe)}'
      + '.raw{background:#f6f7fb;border:1px solid #e4e7eb;border-radius:8px;padding:12px;white-space:pre-wrap;font-family:ui-monospace,Menlo,Consolas,monospace;font-size:12px;word-break:break-all}'
      + '.code{font-size:22px;font-weight:700;color:#6c5ce7;letter-spacing:3px}'
      + '.note{font-size:13px;color:#6b7280;background:#fff8e6;border-left:3px solid #f0a020;padding:7px 11px;border-radius:4px}'
      + '.decl{font-size:13px;color:#4b5563;background:#f3f4ff;border:1px solid #dcdcff;border-radius:8px;padding:12px 15px;margin-top:22px}'
      + '.viz{display:flex;gap:18px;flex-wrap:wrap;justify-content:center;margin:10px 0}'
      + '.rescards{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:10px;margin:10px 0}'
      + '.rescard{background:#f1f8f4;border:1px solid #bfe1cb;border-radius:10px;padding:10px 12px;font-size:13px;color:#0f5132}'
